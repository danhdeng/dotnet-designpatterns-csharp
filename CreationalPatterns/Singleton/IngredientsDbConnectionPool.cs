...
public class IngredientsDbConnectionPool {
    ...
    public async Task Connect(string client) {
        if (_openConnections >= Configuration.MaxConnections) {
            _logger.LogError($"Error- Cannnot aquire new connection. Max connections of {Configuration.MaxConnections} is met or exceeded");
            return;
         }
        _openConnections++;
    ...
    public async Task Disconnect() {
        ...
        _logger.LogInfo($"Release connection. Now Managing ({_openConnections}) open connections", ConsoleColor.Blue);
